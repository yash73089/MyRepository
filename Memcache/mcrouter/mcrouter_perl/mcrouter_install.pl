...
my $guest_config_script = '/home/mcrouter_install.sh'.time.$ARGV[1];
my $host_config_script = '/usr/local/netmagic/cloud/bin/shell_scripts/memcached/mcrouter/mcrouter_install.sh';
my $guest_getip_file = '/home/mcrouter.tar.gz';
my $host_getip_file = '/usr/local/netmagic/cloud/bin/shell_scripts/memcached/mcrouter/mcrouter.tar.gz';
my $guest_output_file = "/home/mcinstall.txt"; 
my $host_output_file = "/usr/local/netmagic/cloud/bin/shell_scripts/memcached/mcrouter/temp/mcinstall_$cloud_app_component_id.txt";
#my $guest_getip_file = '/home/mcrouter-v1.tar.gz';
#my $host_getip_file = '/usr/local/netmagic/cloud/bin/shell_scripts/memcached/mcrouter/mcrouter-v1.tar.gz';
  ...
  $err = VMCopyFileFromHostToGuest ($vmHandle,
   ...
   die "VMCopyFileFromHostToGuest() failed, $err ", GetErrorText($err), " \n" if $err != VIX_OK;
	...
	$err = VMRunProgramInGuest(
            ...
            " >> /home/mcinstall.txt" ,                     # arguements
            0,                                         # options
            VIX_INVALID_HANDLE                         # propertyListHandle
        );

   $err = VMCopyFileFromGuestToHost ($vmHandle,
                                    "$guest_output_file",     # src name
                                    "$host_output_file",   # dest name
                                    0,                     # options
                                    VIX_INVALID_HANDLE);   # propertyListHandle
...
$var = `tail -1 $host_output_file | grep -o 'Mcrouter installed'`;
if ($var =~ 'Mcrouter installed'){
        print "$cloud_app_component_id:Success,\n";
}
else
{
        print "Failed";
