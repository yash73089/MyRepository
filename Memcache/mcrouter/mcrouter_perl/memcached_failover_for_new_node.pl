...
my $guest_config_script = '/home/memcached_failover_for_new_node.sh'.time.$ARGV[1];
my $host_config_script = "/usr/local/netmagic/cloud/bin/shell_scripts/memcached/mcrouter/temp/memcached_failover_for_new_node_$cloud_app_component_id.sh";
my $guest_getip_file = '/usr/local/mcrouter/failover.json';
my $host_getip_file = "/usr/local/netmagic/cloud/bin/shell_scripts/memcached/mcrouter/temp/failover_$cloud_app_component_id.json";
my $guest_output_file = "/home/memcached_failover_for_new_node.txt"; 
my $host_output_file = "/usr/local/netmagic/cloud/bin/shell_scripts/memcached/mcrouter/temp/memcached_failover_for_new_node_$cloud_app_component_id.txt";
  ...
  $err = VMCopyFileFromHostToGuest ($vmHandle,
   ...
   die "VMCopyFileFromHostToGuest() failed, $err ", GetErrorText($err), " \n" if $err != VIX_OK;
	...
	$err = VMRunProgramInGuest(
            ...
            " >> /home/memcached_failover_for_new_node.txt",                     # arguements
            0,                                         # options
            VIX_INVALID_HANDLE                         # propertyListHandle
        );
...
$var = `tail -1 $host_output_file | grep -o 'Success'`;
if ($var =~ 'Success'){
        print "Success\n";
}
else
