...
my $guest_config_script = '/home/memcached_failover.sh'.time.$ARGV[1];
my $host_config_script = "/usr/local/netmagic/cloud/bin/shell_scripts/memcached/mcrouter/temp/memcached_failover_$cloud_app_component_id.sh";
...
my $guest_start_mcrouter_file = '/home/start_mcrouter.sh';
my $host_start_mcrouter_file = "/usr/local/netmagic/cloud/bin/shell_scripts/memcached/mcrouter/temp/start_mcrouter_$cloud_app_component_id.sh";
...
die "VMWaitForToolsInGuest() failed, $err ", GetErrorText($err), "\n" if $err != VIX_OK;
  ...
  $err = VMCopyFileFromHostToGuest ($vmHandle,
   ...
   $err = VMCopyFileFromHostToGuest ($vmHandle,
                                    "$host_start_mcrouter_file",     # src name
                                    "$guest_start_mcrouter_file",   # dest name
                                    0,                     # options
                                    VIX_INVALID_HANDLE);   # propertyListHandle
   die "VMCopyFileFromHostToGuest() failed, $err ", GetErrorText($err), " \n" if $err != VIX_OK;
	...
	$err = VMRunProgramInGuest(
            ...
            " >> /home/memcached_failover.txt",                     # arguements
            0,                                         # options
            VIX_INVALID_HANDLE                         # propertyListHandle
        );
