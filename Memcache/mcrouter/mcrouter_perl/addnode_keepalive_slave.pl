...
my $guest_config_script = '/home/keepalive_slave.sh'.time.$ARGV[1];
my $host_config_script = '/usr/local/netmagic/cloud/bin/shell_scripts/memcached/mcrouter/keepalive_slave.sh';
my $guest_getip_file = '/home/keepalive_conf_slave.txt';
my $host_getip_file = "/usr/local/netmagic/cloud/bin/shell_scripts/memcached/mcrouter/temp/keepalive_conf_slave_$cloud_app_component_id.txt";
...
my $guest_output_file = "/home/addkeepalive.txt";
my $host_output_file = "/usr/local/netmagic/cloud/bin/shell_scripts/memcached/mcrouter/temp/addkeepalive_$cloud_app_component_id.txt";
  ...
  $err = VMCopyFileFromHostToGuest ($vmHandle,
   ...
   die "VMCopyFileFromHostToGuest() failed, $err ", GetErrorText($err), " \n" if $err != VIX_OK;
	...
	$err = VMRunProgramInGuest(
            ...
            " >> /home/addkeepalive.txt" ,                     # arguements
            0,                                         # options
            VIX_INVALID_HANDLE                         # propertyListHandle
        );
...
$var = `tail -1 $host_output_file | grep -o 'done'`;
if ($var =~ 'done'){
